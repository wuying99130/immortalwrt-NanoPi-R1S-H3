以下为您更新了 **编译的思路和原理** 章节的完整版 `README.md` 内容。您可以直接复制并替换您仓库中的 `README.md` 文件：
...
1. **零源码沉重负担**：本地与 Git 仓库中仅保留几 KB 的核心配置、补丁和脚本，不存储体积庞大的上游源码树。
2. **云端动态解耦**：当在 GitHub Actions 触发编译时，云端虚拟机自动克隆一份干净的官方 `openwrt-24.10` 稳定分支。
3. **精准硬件注入**：通过 `patch-dts.sh` 脚本自动将手写的底层设备树（DTS）和设备条目塞入内核，完美适配 NanoPi R1S-H3 的网卡、LED 指示灯及存储驱动。
4. **规范化固件产出**：编译完成后，自动提取镜像并统一规范命名，整洁存放至 `bin/out/` 目录供下载。
...
1. 进入仓库的 **Actions** 页面。
2. 选择 **Build ImmortalWrt NanoPi R1S-H3** 工作流。
3. 点击 **Run workflow** 手动触发云端编译。
4. 编译结束后，直接在页面下方的 **Artifacts** 处下载打包好的固件。
